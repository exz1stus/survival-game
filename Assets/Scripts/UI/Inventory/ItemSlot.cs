...
public class ItemSlot
{
    ...
    public static void TakeAHalf(ItemSlot givingSlot, ItemSlot takingSlot)
    {
        if (givingSlot.amount > 0)
        {
            int divVlaue = Mathf.RoundToInt(givingSlot.amount / 2);
            if (!takingSlot.hasItem) takingSlot.item = givingSlot.item;

            takingSlot.condition = givingSlot.condition;

            givingSlot.amount -= divVlaue;
            takingSlot.amount += divVlaue;

            // (givingSlot.amount <= 0) givingSlot.Clear(); //

            givingSlot.RefreshUISlot();
            takingSlot.RefreshUISlot();
        }
    }
    public static void GiveOne(ItemSlot givingSlot, ItemSlot takingSlot)
    {
        if (givingSlot.amount > 0)
        {
            if (!takingSlot.hasItem) takingSlot.item = givingSlot.item;
    ...
    public ItemSlot(string itemName, int _amount = 1, int _condition = 0)
    {
        ...
        if (_item == null)
        { // If item is null for whatever reason, slot is empty.
